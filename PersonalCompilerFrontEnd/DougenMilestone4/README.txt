...
In this update an if statement, else statment, and while loop statement were added to 
compiler. Becuase each statement and loop presented similar problems, I will be splitting 
this assessment up by lexer, parser, AST rather than by the statements.

The lexer presented a few problems at first. I was having trouble getting the lexer to
differentiate between a staement and an id. The words "if" and "while" would be read as
id's before they were read as statements. This was a problem until I realized I should
put the statements in the reserved words dictionary. That solved the issue. 

Next, I had to incorporate the statements into the parser. This was probably the easiest
part of the assignment. I simply created two new stmts, IF and WHILE, the created an
optelse for the else statment. To differentiate between If and IF ELSE, I added a small
if-statement that checked to see if optelse was empty or not. If it was, the information was
sent to the IfNode., If not, it was sent to the IfElseNode. The code looked like this:
...
I also solved the issue of the ambiguous if-else statment. Every IF had to start with and 
end with parnethesis that captured the statement inside. An example would look like this:

if x(
 x = y;
)

The parenthesis would tell the compiler if an else statement was inside of it or next to it.
In this example, the ELSE belongs to the first IF:
...
In this one, the ELSE belongs to the second IF:
...
This can be seen in action in the test4 file.

Finally, I had to incorporate the statements into the AST. Three new nodes had to be created
to do this: the IfNode, IfElseNode, and WhileNode. All of these functioned similarly because
they shared the same "IfFalse expr GOTO" format. I wrestled with generating code in the right
order during this part. I eventually settled on code that first generated the statement's
condition, then the "IfFalse expr GOTO", and finally the remaining code. At the end of this,
the label that the GOTO pointed to was printed to show that's where the code should continue.

Test1 shows how the compiler handles an if statement. Test2 shows how it handles an if-else
statement. Test3 shows how it handles a while loop.

As a final note, I also added comment and multi-line comment support to the parser. They
don't go to the AST, but the parser does recognize them now.
