...
class IfNode(ExprNode):
	...
	def gen(self):
		...
		#generate a new temporary variable for placing the result of this ASTNode
                self.set_addr( NameGenerator.new_temp() )
                new_label = NameGenerator.new_label()
                #emit the three address code
                print('ifFalse', self.__expr.get_addr(), 'goto' , new_label)
                self.__stmt.gen()
                print(new_label)
...
class AssignNode(ASTNode):
	...
	def __init__(self,varname,exp):
		...
		self.__exprs = exp    #right hand side of assignment statement

	def gen(self):
                self.__exprs.gen() #generate code for the value we're assigning
                #three address code for assigning the value to the rhs ID
                print(self.__var,'=',self.__exprs.get_addr())
                        

class UnaryNode(ExprNode):

        def __init__(self, unary, factor):
                ASTNode.__init__(self)
                self.__unary = unary
        ...
        def gen(self):
                self.__factor.gen()
                self.set_addr( NameGenerator.new_temp() )
                print(self.get_addr(), "=", self.__unary, self.__factor.get_addr())
